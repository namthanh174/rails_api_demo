class Api::V1::ReviewsController < ApplicationController
  ...
  def index
    ...
    reviews_rerializer = parse_json @reviews
    json_response "Index review successfully", true, {reviews: reviews_rerializer}, :ok
  end

  def show
    review_rerializer = parse_json @review
    json_response "Show review", true, {review: review_rerializer}, :ok
  end
  ...
  def create
    ...
    if review.save
      review_rerializer = parse_json @review
      json_response "Created review successfully", true, {review: review_rerializer}, :ok
    else
  ...
  def update
    if correct_user @review.user
      if @review.update review_params
        review_rerializer = parse_json @review
        json_response "Updated review successfully", true, {review: review_rerializer}, :ok
      else
